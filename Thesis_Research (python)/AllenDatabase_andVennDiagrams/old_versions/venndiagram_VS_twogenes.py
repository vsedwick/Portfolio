...
combo = 'crfr2-aromatase'
dataset = 'CTXsp_dataset'
...
def venndiagrams(df, item, name, file_save):
    for i, class_name in enumerate(item):
        ...
        # Plot Venn diagram
        venn2([set_gene1, set_gene2], (gene1, gene2))
...
def colocalizations(df, region, file_save):
    ...
    for i in gene_list:
        ...
        labels = (i, f'{i} & {gene_listx[0]}')
        arrays = [main_gene, main_2gene]
        # Plot Venn diagram
        v = venn2(arrays, labels)

        if labels[0] == gene1:
            # Set custom colors
            v.get_patch_by_id('10').set_color((1, 0, 0, 0.5))  # Circle for the first gene
            p = (len(arrays[0]) - len(arrays[0] & arrays[1])) * 100 / len(arrays[0])
            v.get_label_by_id('10').set_text(f"{p:.2f}%")

            v.get_patch_by_id('01').set_color((0, 1, 0, 0.5))  # Circle for the second gene
            p = (len(arrays[1]) - len(arrays[0] & arrays[1])) * 100 / len(arrays[0])
            v.get_label_by_id('01').set_text(f"{p:.2f}%")

            p = (len(arrays[0] & arrays[1])) * 100 / len(arrays[0])
            v.get_label_by_id('11').set_text(f"{p:.2f}%")

        elif labels[0] == gene2:
            # Set custom colors
            v.get_patch_by_id('10').set_color((0, 1, 0, 0.5))  # Circle for the first gene
            p = (len(arrays[0]) - len(arrays[0] & arrays[1])) * 100 / len(arrays[0])
            v.get_label_by_id('10').set_text(f"{p:.2f}%")

            v.get_patch_by_id('01').set_color((1, 0, 0, 0.5))  # Circle for the second gene
            p = (len(arrays[1]) - len(arrays[0] & arrays[1])) * 100 / len(arrays[0])
            v.get_label_by_id('01').set_text(f"{p:.2f}%")
